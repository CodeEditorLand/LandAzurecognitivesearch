	...
	await callWithTelemetryAndErrorHandling(
		...
		async (activateContext: IActionContext) => {
			...
			registerCommand(
				...
				async (
					actionContext: IActionContext,
					treeItem: DataSourceListTreeItem,
				) =>
					createResource(
						treeItem,
						actionContext,
			...
			registerCommand(
				...
				async (
					actionContext: IActionContext,
					treeItem: EditableResourceTreeItem,
				) =>
					deleteResource(
						treeItem,
						actionContext,
			...
			registerCommand(
				...
				async (
					actionContext: IActionContext,
					treeItem: SynonymMapListTreeItem,
				) =>
					createResource(
						treeItem,
						actionContext,
			...
			registerCommand(
				...
				async (
					actionContext: IActionContext,
					treeItem?: AzureTreeItem,
				) => {
					if (!treeItem) {
						treeItem = <SearchServiceTreeItem>(
					...
					const id = (<SearchServiceTreeItem>treeItem).searchService
						.id;
					if (id !== undefined) {
			...
			registerCommand(
				...
				async (
					...
					if (!treeItem) {
						treeItem = await ext.tree.showTreeItemPicker(
							IndexerListTreeItem.itemContextValue,
							actionContext,
						);
					}
					...
					const r = await vscode.window.showWarningMessage(
						`Are you sure you want to reset ${treeItem.itemName} ${treeItem.itemKind}?`,
						DialogResponses.yes,
			...
			registerCommand(
				...
				async (
					...
					const r = await vscode.window.showWarningMessage(
						`Are you sure you want to run ${treeItem.itemName} ${treeItem.itemKind}?`,
						DialogResponses.yes,
						DialogResponses.cancel,
					);
					if (r === DialogResponses.yes) {
			...
			registerCommand(
				...
				async (actionContext: IActionContext, node?: AzureTreeItem) => {
					...
					const r = await vscode.window.showWarningMessage(
						`Are you sure you want to delete the search service ${node.label}?`,
						DialogResponses.yes,
						DialogResponses.cancel,
					);
					if (r === DialogResponses.yes) {
			...
			vscode.commands.registerTextEditorCommand(
				"azureCognitiveSearch.searchDoc",
				(editor) =>
					searchToDocument(
						editor,
						azureAccountTreeItem,
			...
			registerEvent(
				"azureCognitiveSearch.searchDocument.onDidSaveTextDocument",
				vscode.workspace.onDidSaveTextDocument,
				async (
					_actionContext: IActionContext,
					doc: vscode.TextDocument,
				) => await documentEditor.onDidSaveTextDocument(doc),
			);

			registerEvent(
				"azureCognitiveSearch.searchResults.onDidCloseTextDocument",
...
async function deleteResource(
	treeItem: AzExtTreeItem & IDocumentRepository,
	actionContext: IActionContext,
	contextValue: string,
): Promise<void> {
	if (!treeItem) {
	...
	const r = await vscode.window.showWarningMessage(
		`Are you sure you want to delete this ${treeItem.itemKind}?`,
...
async function search(
	...
	if (!indexItem) {
		indexItem = <IndexTreeItem>(
			await ext.tree.showTreeItemPicker(
				IndexTreeItem.contextValue,
				actionContext,
			)
		);
	}

	const query = await ext.ui.showInputBox({
...
async function searchToDocument(
...
async function copyAdminKey(treeItem: SearchServiceTreeItem): Promise<void> {
	const keys = await treeItem.getAdminKeys();
...
async function copyQueryKey(treeItem: SearchServiceTreeItem): Promise<void> {
	const keys = await treeItem.getQueryKey();
